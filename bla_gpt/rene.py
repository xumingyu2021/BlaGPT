...
class ReneMLP(nn.Module):
    ...
    def __init__(
        self, d_input, d_output=None, expand=3, bias=True, device=None, dtype=None
    ):
        super().__init__()
        ...
        self.in_proj = nn.Linear(
            self.d_input, self.d_inner, bias=bias, **factory_kwargs
        )
        self.activation = nn.GELU()
        self.out_proj = nn.Linear(
            self.d_inner, self.d_input, bias=bias, **factory_kwargs
        )
...
class ReneMHA(nn.Module):
    ...
    def __init__(
        ...
        self.out_proj = nn.Linear(
            out_dim, embed_dim, bias=out_proj_bias, **factory_kwargs
        )
...
def _create_block(
    ...
    return Block(
        d_model, mixer_cls, norm_cls=norm_cls, residual_in_fp32=residual_in_fp32
    )
    ...
    def forward(
        ...
        **mixer_kwargs,
        ...
        hidden_states = self.backbone(
            idx, inference_params=inference_params, **mixer_kwargs
        )
