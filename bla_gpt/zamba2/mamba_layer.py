...
class MambaLayer(nn.Module):
    def __init__(self, config: MambaConfig, layer_idx=0):
        factory_kwargs = {}
        super().__init__()
        self.config = config
        ...
        self.dt_rank = (
            math.ceil(self.d_model / 16) if config.dt_rank == "auto" else config.dt_rank
        )
        self.use_fast_path = (
            config.use_fast_path if mamba_inner_fn is not None else False
        )
        ...
        self.out_proj = nn.Linear(
            self.d_inner, self.d_model, bias=config.add_bias_linear, **factory_kwargs
        )
