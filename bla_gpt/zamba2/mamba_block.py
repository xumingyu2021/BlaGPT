...
try:
    from zamba2.ops.selective_scan_interface import mamba_inner_fn, selective_scan_fn
except ImportError:
    selective_scan_fn, mamba_inner_fn = None, None

try:
    from zamba2.ops.triton.selective_state_update import selective_state_update
except ImportError:
    selective_state_update = None

try:
    from zamba2.ops.triton.layernorm import RMSNorm, layer_norm_fn, rms_norm_fn
except ImportError:
    RMSNorm, layer_norm_fn, rms_norm_fn = None, None, None

from zamba2.attention import CausalSelfAttention
from zamba2.config import MambaConfig
from zamba2.mamba2_layer import Mamba2Layer
from zamba2.mamba_layer import MambaLayer
from zamba2.mlp import MLP
from zamba2.rotary import RotaryEmbedding
...
class MambaBlock(nn.Module):
    def __init__(
        ...
        if self.fused_add_norm:
            ...
            assert (
                config.num_mem_heads == 0
            ), "args.num_mem_heads > 0 only supports fused_add_norm=False"
        ...
        if not self.fused_add_norm:
            ...
            if from_tf is not None:
                hidden_states = self.norm(
                    (residual + from_tf).to(dtype=self.norm.weight.dtype)
                )
            else:
...
class Memory_AttentionBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mixer(
            ...
            attention_mask=attention_mask,
            inference_params=inference_params,
            forward_layer_idx=forward_layer_idx,
        )
...
def create_block(config, layer_idx):
    ...
    if layer_idx == -1:
        ...
        mlp_cls = partial(
            MLP, layer_idx=layer_idx, **factory_kwargs, num_mem_blocks=num_gs
        )
        block = vBlock(
            config,
            sa_cls=sa_cls,
        ...
        if (
            (not config.layer_mapping)
            or config.layer_mapping[layer_idx - 1][0] == "r"
            or config.layer_mapping[layer_idx - 1][0] == "g"
        ):
            if (not config.layer_mapping) or len(
                config.layer_mapping[layer_idx - 1]
            ) == 1:
                if "m" in config.layer_mapping:
                    mixer_cls = partial(
                        Mamba2Layer, layer_idx=layer_idx, **factory_kwargs
                    )
                else:
                    mixer_cls = partial(
                        MambaLayer, layer_idx=layer_idx, **factory_kwargs
                    )
                block = MambaBlock(
                    config,
                    mixer_cls=mixer_cls,
...
class MambaDecoder(nn.Module):
    def __init__(
        ...
        self.num_layers_per_pipeline_rank = self.config.num_layers
    ...
    def _build_layers(self):
        ...
        self.layers = torch.nn.ModuleList(
            [create_block(self.config, i + 1) for i in range(num_layers_to_build)]
        )
        if self.config.num_mem_heads > 0:
            ...
            if self.use_mem_rope:
                self.rotary_pos_emb = RotaryEmbedding(
                    2 * self.config.hidden_size // self.config.num_mem_heads,
                    rotary_percent=1.0,
        ...
        if self.config.use_low_rank_mamba_proj:
            ...
            d_in_proj = (
                2 * d_inner
                + 2 * self.config.mamba_ngroups * self.config.state_size
                + nheads
            )
    ...
    def forward(
        self, hidden_states, residual=None, inference_params=None, attention_mask=None
    ):
        ...
        with rng_context and fp8_context:
            ...
            for i, layer in enumerate(self.layers):
                ...
                if self.config.use_low_rank_mamba_proj:
                    from_shared_proj = self.in_projs[
                        i % self.config.num_shared_mamba_proj
                    ](hidden_states)
        ...
        if self.post_process and self.post_layer_norm:
            if not self.config.fused_add_norm:
                ...
                hidden_states = self.final_layernorm(
                    residual.to(dtype=self.final_layernorm.weight.dtype)
                )
            else:
                fused_add_norm_fn = (
                    rms_norm_fn
                    if isinstance(self.final_layernorm, RMSNorm)
                    else layer_norm_fn
                )
