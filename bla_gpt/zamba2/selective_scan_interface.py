...
global SELECTIVE_SCAN_CUDA_IMPORT_FAILED
SELECTIVE_SCAN_CUDA_IMPORT_FAILED = False
try:
    import selective_scan_cuda
except Exception as e:
    SELECTIVE_SCAN_CUDA_IMPORT_FAILED = True
    print("SELECTIVE SCAN CUDA IMPORT FAILED: ", e)
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(
        ...
        # print("xz",xz.shape, "conv1dweight", conv1d_weight.shape, "conv1dbias: ", conv1d_bias.shape, "xproj: ", x_proj_weight.shape, "delta proj: ", delta_proj_weight.shape)
        # print("DELTA PROJ WEIGHT: ", delta_proj_weight.shape)
        ...
        ctx.save_for_backward(
            ...
            x_dbl,
            x_proj_weight,
            delta_proj_weight,  ## out_proj_weight,
            conv1d_out,
            delta,
            A,
            B,
    ...
    def backward(ctx, dout):
        ...
        # x, z = xz.chunk(2, dim=1)
        x, z = xz.view(batch, ctx.d_inner, 2, L).chunk(2, dim=2)
        x = x.squeeze(2)
        z = z.squeeze(2)
        # if dout.stride(-1) != 1:
        #     dout = dout.contiguous()
        ...
        dx, dz = dxz.view(batch, ctx.d_inner, 2, L).chunk(2, dim=2)
        dx = dx.squeeze(2)
        dz = dz.squeeze(2)
        ...
        ## dout_proj_bias = dout.sum(dim=(1,2)) if not ctx.out_proj_bias_is_None else None
        ...
        # print("MAMBA_INNER_FN_BACKWARD: ")
        # print("DXZ: ", dxz.shape, dxz.sum().item())
        # print("dconv1d_weight", dconv1d_weight.shape,dconv1d_weight.sum().item())
        # print("dx_proj_weight",dx_proj_weight.shape,dconv1d_weight.sum().item())
        # print("ddelta_proj_weight",ddelta_proj_weight.shape,ddelta_proj_weight.sum().item())
        # print("dout_proj_weight", dout_proj_weight.shape,dout_proj_weight.sum().item())
        # print("dout_proj_bias", dout_proj_bias.shape,dout_proj_bias.sum().item())
        # print("dA", dA.shape,dA.sum().item())
        # print("dB", dB.shape if dB is not None else "NONE!!!")
        # print("dC", dC.shape if dC is not None else "NONE!!!")
        # print("dD",dD.shape if dD is not None else "NONE!!!")
        # #die
        return (
            None,
            None,
