...
from zamba2.ops.selective_scan_interface import (
    SELECTIVE_SCAN_CUDA_IMPORT_FAILED,
    mamba_inner_fn,
    selective_scan_fn,
)
...
from zamba2.ops.triton.layernorm_gated import RMSNorm as RMSNormGated
from zamba2.ops.triton.ssd_combined import (
...
class Mamba2Layer(nn.Module):
    def __init__(
        ...
        if self.config.use_low_rank_mamba_proj:
            self.in_proj = nn.ModuleList(
                [
                    nn.Linear(self.d_model, self.config.mamba_lora_rank),
                    nn.Linear(self.config.mamba_lora_rank, d_in_proj),
                ]
            )
        else:
            self.in_proj = nn.ModuleList(
    ...
    def forward(
        self, u, from_shared_proj=None, seqlen=None, seq_idx=None, inference_params=None
    ):
        """
    ...
    def allocate_inference_cache(
        self, batch_size, max_seqlen, dtype=torch.bfloat16, **kwargs
    ):
    ...
    def _get_states_from_cache(
        ...
        if self.layer_idx not in inference_params.key_value_memory_dict_mamba:
            ...
            inference_params.key_value_memory_dict_mamba[self.layer_idx] = (
                conv_state,
                ssm_state,
            )
        else:
            conv_state, ssm_state = inference_params.key_value_memory_dict_mamba[
                self.layer_idx
            ]
